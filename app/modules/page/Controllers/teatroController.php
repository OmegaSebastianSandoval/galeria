...
class Page_teatroController extends Page_mainController
{

    public function indexAction()
    {
        //  $publicidadModel = new Page_Model_DbTable_Publicidad();
        // $this->_view->banner = $publicidadModel->getList("publicidad_seccion='3'","")[0];
        
        $this->_view->banner = $this->template->bannerprincipal(3);
        $programacionModel = new Page_Model_DbTable_Teatro();
        $fechaactualevento = date('Y-m-d H:i:s');
        $filters = " '$fechaactualevento' <= programacion_fecha  AND programacion_tipoevento = 'Teatro'";
