 ...
 */
class Administracion_Model_DbTable_Transacciones extends Db_Table
{
	/**
	...
	public function getListSum($filters = '', $order = '')
	{
		$filter = '';
		if ($filters != '') {
		...
		$select = 'SELECT SUM(t.boleta_compra_cantidad) as cantidad_total, t.vendor, t.boleta_compra_tipoboleta, p.programacion_nombre, p.programacion_fecha
		FROM boleta_compra t 
		INNER JOIN boleta_evento b
		ON   b.boleta_evento_id =  t.boleta_compra_tipoboleta
		INNER JOIN programacion p 
		ON p.programacion_id  = b.boleta_evento_evento 
		' . $filter .  ' AND t.validacion = 1 
        GROUP BY t.vendor, t.boleta_compra_tipoboleta
        ORDER by t.boleta_compra_id DESC';
		$res = $this->_conn->query($select)->fetchAsObject();
		return $res;
	}
	public function getListPagesSum($filters = '', $page, $amount)
	{
		$filter = '';
		if ($filters != '') {
			$filter = ' WHERE ' . $filters;
		...
		INNER JOIN programacion p 
		ON p.programacion_id  = b.boleta_evento_evento 
		' . $filter .  ' AND t.validacion = 1  GROUP BY t.vendor, t.boleta_compra_tipoboleta   ORDER by t.boleta_compra_id DESC '  . 
		' LIMIT ' . $page . ' , ' . $amount;
		$res = $this->_conn->query($select)->fetchAsObject();
		return $res;
	}
...
public function getListSum($filters = '', $order = '')
{
	...
	$select = 'SELECT SUM(t.boleta_compra_cantidad) as cantidad_total, t.vendor, t.boleta_compra_tipoboleta 
	FROM ' . $this->_name .  ' t 
	INNER JOIN boleta_evento b
	ON  t. = b.boleta_evento ' . $filter . '  GROUP BY vendor, boleta_compra_tipoboleta'  . $orders;
	$res = $this->_conn->query($select)->fetchAsObject();
	return $res;
} */