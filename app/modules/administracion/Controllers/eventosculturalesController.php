 ...
 */
class Administracion_eventosculturalesController extends Administracion_mainController
{
	public $botonpanel = 33;
	...
	public function init()
	{
		...
		$this->mainModel = new Administracion_Model_DbTable_EventosCulturales();
		$this->namefilter = "parametersfiltereventosculturales";
		$this->route = "/administracion/eventosculturales";
		$this->namepages = "pages_eventosculturales";
		$this->namepageactual = "page_actual_eventosculturales";
	...
	{
		$title = "Administración de Eventos Culturales	";
		...
		$order = "orden ASC";
		$list = $this->mainModel->getList("$filters AND programacion_tipoevento = 'Eventos'", $order);
		$amount = $this->pages;
		...
		$this->_view->lists = $this->mainModel->getListPages("$filters AND programacion_tipoevento = 'Eventos'", $order, $start, $amount);
	...
	public function manageAction()
	{
		...
		if ($id > 0) {
			...
			if ($content->programacion_id) {
				...
				$title = "Actualizar Evento cultural";
				$this->getLayout()->setTitle($title);
				$this->_view->titlesection = $title;
			} else {
				$this->_view->routeform = $this->route . "/insert";
				$title = "Crear Evento cultural";
				$this->getLayout()->setTitle($title);
				$this->_view->titlesection = $title;
			}
		} else {
	...
	public function enlaceAction()
	{
		...
		$texto = "/programacion/detalle?vendor=" . $vendor . "&id=" . $id;
